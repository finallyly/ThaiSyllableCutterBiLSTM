...
class LSTMCell(object):
    ...
    def __init__(self,n_input,n_hidden,stddev=None,variable_values=None,name="GRU"):
        if variable_values is None:
            #input gate
            self.W_i = weight_variable_uniform([n_input+n_hidden,n_hidden],stddev);
            self.b_i = tf.Variable(tf.zeros(n_hidden,tf.float32));
            # forget gate
            self.W_f = weight_variable_uniform([n_input + n_hidden, n_hidden], stddev);
            self.b_f = tf.Variable(tf.zeros(n_hidden,tf.float32));
            #output gate
            self.W_o = weight_variable_uniform([n_input + n_hidden, n_hidden], stddev);
            self.b_o = tf.Variable(tf.zeros(n_hidden,tf.float32));
            # memory cell
            self.W_c = weight_variable_uniform([n_input+n_hidden,n_hidden],stddev);
            self.b_c = tf.Variable(tf.zeros(n_hidden,tf.float32));
        else:
            self.W_i = tf.Variable(variable_values[":".join([name,"W_i"])]);
            self.b_i = tf.Variable(variable_values[":".join(name,"b_i")]);
            self.W_f = tf.Variable(variable_values[":".join([name,"W_f"])]);
            self.b_f = tf.Variable(variable_values[":".join([name,"b_f"])]);
            self.W_o = tf.Variable(variable_values[":".join([name,"W_o"])]);
            self.b_o = tf.Variable(variable_values[":".join([name,"b_o"])]);
            self.W_c = tf.Variable(variable_values[":".join([name,"W_c"])]);
            self.b_c = tf.Variable(variable_values[":".join([name,"b_c"])]);
        self.n_input = n_input;
        self.n_hidden = n_hidden;
        self.variables={":".join([name,"W_i"]):self.W_i,
                        ":".join([name, "b_i"]):self.b_i,
                        ":".join([name,"W_f"]):self.W_f,
                        ":".join([name, "b_f"]): self.b_f,
                        ":".join([name, "W_o"]): self.W_o,
                        ":".join([name, "b_o"]): self.b_o,
                        ":".join([name,"W_c"]): self.W_c,
                        ":".join([name,"b_c"]): self.b_c
                        }


    def __call__(self,h,x,c):
        ...
        new_c=f*c+i*tf.tanh(tf.matmul(hx,self.W_c)+self.b_c);
...
def build_encoder_layers(input,n_step_input,encoder_layers,reverse_input=False):
    ...
    for l in range(n_layer):
        ...
        z_init = tf.zeros((n_sample,encoder.n_hidden),tf.float32);
        states=[];
        memories=[];
        for t in range(n_step_input):
            h_prev = h_init if t==0 else states[-1];
            z_prev = z_init if t==0 else memories[-1];
            input_t = states_prev[t];
            h_t,z_t = encoder(h_prev,input_t,z_prev);
            states.append(h_t);
            memories.append(z_t);
        states_layers.append(states);
        memory_layers.append(memories);
...
def main():
    ...
    for l in range(n_layer):
        ...
        encoder = LSTMCell(input_size,layer_size,weight_stddev,name='encoder:{}'.format(l));
        encoder_r =LSTMCell(input_size,layer_size,weight_stddev,name='encoder_r:{}'.format(l));
